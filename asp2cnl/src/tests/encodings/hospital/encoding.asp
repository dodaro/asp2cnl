...
%% res(RID,DAY,TS..TS+PH4-1) :- x(RID,DAY,TS,PH4,_,_), PH4 > 0. chair4(ID,RID,DAY,TS)
:- chair3(ID,RID,DAY), res(RID,DAY,TS). bed4(ID,RID,DAY,TS)
:- bed3(ID,RID,DAY), res(RID,DAY,TS).
:- #count{RID: chair4(ID,RID,DAY,TS)} > 1, day(DAY), ts(TS), chair(ID).
:- #count{RID: bed4(ID,RID,DAY,TS)} > 1, day(DAY), ts(TS), bed(ID).
%% support(RID,DAY,TS) :- x(RID,DAY,PH4,_,_,_), registration(RID,ORDER,_,_,PH3,PH2,_,_), PH2 > 0, TS=PH4-PH3-PH2, day(DAY), ats(TS).
...
%% :- #count{RID: nurses4(ID,RID,DAY,TS)} > K, day(DAY), ts(TS), nurse(ID), nurseLimits(K).
%% :- drug(DRUG,LMT,DAY), #count{RID: x(RID,DAY,_,_,ORDER,_), registration(RID,_,ORDER,_,_,_,_,_,_,DRUG)} > LMT.
%% :- x(RID,DAY,36,_,PH4,_,_).
%% :∼ registration(RID,0,_,_,_,_,_,_,1,_), x(RID,_,TS,_,_,_). [TS@3,RID]
%% :∼ registration(RID,0,_,_,_,_,_,_,2,_), x(RID,_,TS,_,_,_). [TS@3,RID]
%% :∼ registration(RID,0,_,_,_,_,_,_,3,_), x(RID,_,TS,_,_,_). [TS@3,RID]
