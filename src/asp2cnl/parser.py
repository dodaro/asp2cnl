...
class ASPParser:
    __aspCoreParser = Lark(
        open(os.path.join(os.path.dirname(__file__), "asp_core_2_grammar/asp_grammar.lark"), "r").read())

    def __init__(self, programFile: str):
...
class ASPTransformer(Transformer):
    ...
    def term(self, elem):
        ...
        #elif len(elem) == 3:# and elem[0].value == "(" and elem[-1].value == ")":
        #operators = elem[1].ops
                #       print(operators)
                #terms = elem[1].terms
                #return ArithmeticAtom(operators, terms)
...
class Rule:
    ...
    def isFact(self):
        return type(self.head) != Choice and self.head is not None and len(
            self.head.atoms) == 1 and self.body is None and not self.head.atoms[0].hasVariables()

    def isClassical(self):
        return type(self.head) != Choice and self.head is not None and len(self.head.atoms) == 1 and not \
        self.head.atoms[0] == Choice and self.body is not None and len(self.body.literals) > 0

    def isStrongConstraint(self):
        return type(self.head) != Choice and self.head is None and self.body is not None and len(
            self.body.literals) > 0 and self.weight_at_level is None

    def isDisjunctive(self):
        return type(self.head) != Choice and self.head is not None and len(
            self.head.atoms) > 1 and self.body is not None and len(self.body.literals) > 0
    ...
    def isWeakConstraint(self):
        return type(self.head) != Choice and self.head is None and self.body is not None and len(
            self.body.literals) > 0 and self.weight_at_level is not None
